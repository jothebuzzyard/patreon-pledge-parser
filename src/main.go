...
func main() {
	...
	for _, patron := range patrons {
		// Remove lines with "Free" as Tier
		if strings.Contains(patron.Tier, "Free") {
			continue
		}

		// Remove lines where LastChargeStatus is not "Paid"
		if strings.ToLower(strings.TrimSpace(patron.LastChargeStatus)) != "paid" {
			unpaidStatusCount++
			continue
		}

		// Remove lines where Access Expiration has passed
	...
	for _, patron := range filteredPatrons {
	...
	for tier, patrons := range tierGroups {
		// Create safe filename from tier name
